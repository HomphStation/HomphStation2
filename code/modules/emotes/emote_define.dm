...
/decl/emote/proc/replace_target_tokens(var/msg, var/atom/target)
	. = msg
	if(istype(target))
		var/datum/gender/target_gender = GLOB.gender_datums[target.get_visible_gender()] // CHOMPEdit - Globals
...
/decl/emote/proc/replace_user_tokens(var/msg, var/atom/user)
	. = msg
	if(istype(user))
		var/datum/gender/user_gender = GLOB.gender_datums[user.get_visible_gender()] // CHOMPEdit - Globals
