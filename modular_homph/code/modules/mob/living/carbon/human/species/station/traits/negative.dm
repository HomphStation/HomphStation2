...
/datum/trait/negative/blindness
	...
	is_genetrait = TRUE
	hidden = FALSE //Making this so people can't pick it in character select. While a blind character makes senese, not being able to see posts is a massive issue that needs to be addressed some other time.
